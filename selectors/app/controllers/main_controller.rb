class MainController < UIViewController 
  ...
  def viewDidLoad
    ...
    # For the styles :check_in and :leave, see stylesheets/main_stylesheet.rb
    rmq.append(UIButton, :check_in).on(:tap){ check_me_in }
    rmq.append(UIButton, :leave).on(:tap){ leave }
    rmq(:leave).hide  # initially hidden
    
  end
  
  def check_me_in
    # some check in logic here
    rmq(:leave).show
    rmq(:check_in).hide
  end
